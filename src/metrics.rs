...
pub fn sq_correlation(ground_truth: &[f64], predicted: &[f64]) -> f64 {
    ...
    let (res_sum_sq, tot_sum_sq) = ground_truth.iter()
        .zip(predicted.iter())
        .fold((0.0, 0.0), |(r, t), (gt, pr)| (r + (gt - pr).powi(2),
                                              t + (gt - pred_mean).powi(2)));
...
pub fn accuracy<T: PartialEq>(ground_truth: &[T], predicted: &[T]) -> f64 {
    let (correct, all) = ground_truth.iter()
        .zip(predicted.iter())
...
pub fn class_recall<T: PartialEq>(ground_truth: &[T], predicted: &[T], class: &T) -> f64 {
    let (tp, tp_and_fn) = ground_truth.iter()
        .zip(predicted.iter())
        .fold((0, 0), |(t, g), (gt, pr)| {
...
pub fn class_precision<T: PartialEq>(ground_truth: &[T], predicted: &[T], class: &T) -> f64 {
    let (tp, tp_and_fp) = ground_truth.iter()
        .zip(predicted.iter())
        .fold((0, 0), |(t, g), (gt, pr)| {
              let pred_t = pr == class;
              let correct = gt == pr;
...
pub fn class_f_measure<T: PartialEq>(ground_truth: &[T], predicted: &[T], class: &T) -> f64 {
    let (tp, fn_and_fp) = ground_truth.iter()
        .zip(predicted.iter())
        .fold((0, 0), |(t, g), (gt, pr)| {
              let class_t = gt == class;
              let pred_t = pr == class;
              let correct = gt == pr;
              (t + (pred_t && correct) as i32, g + ((class_t || pred_t) && !correct) as i32)
        });
